...
impl <'a> Cadastro<'a> {
    ...
    pub fn adiciona_usuario(&mut self, nome: String, senha: String) {
        let usuario:Usuario = Usuario::new(String::from(nome), String::from(senha));
    ...
    fn autenticar(&self, nome: String, senha: String) -> bool {
        ...
        let index_usuario: isize = self.retorna_indice_usuario(&nome);

        if index_usuario != -1{
            
        let usuario_cadastrado: &Usuario = &self.usuarios[index_usuario as usize];
            if usuario_cadastrado.nome == nome && usuario_cadastrado.senha == sha256(senha){
...
fn main(){
    Cadastro::adiciona_usuario(String::from("pedrao"), String::from("pedrosafado"));
}
